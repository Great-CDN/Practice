// ============================================================================
// http_server.cpp —— 高性能 HTTP 服务器实现（引擎无关，Select/Epoll 共用本文件）
//
// 端点（与原版 origin_server 保持兼容）：
//   GET  /static/N    立即返回 N KB 确定性数据（keep-alive，压测主路径）
//   GET  /live/<name> 每 100ms 推一帧文本，直到客户端断开
//   GET  /slow/N      延时 2 秒后返回 N KB（短连接，观察合并回源用）
//   POST 任意路径     读齐 body 后回显收到的字节数（keep-alive）
//   其他              400 / 404 / 405 / 413，发完响应即断开
//
// 相对原版的关键改动（均为性能/正确性服务）：
//   1. keep-alive：HTTP/1.1 默认复用连接（原版每次响应后关闭），
//      省掉每请求一次 TCP 握手 + TIME_WAIT 堆积，是吞吐和延迟的大头
//   2. 静态响应体按大小缓存复用（原版每个请求重新生成一遍）
//   3. TCP_NODELAY + 大 backlog，去掉每请求日志（printf+fflush 是吞吐杀手）
//   4. 延迟删除连接对象：回调栈里绝不 delete this，Tick() 末尾统一回收，
//      杜绝"回调中对象悬空"一类事故（原版靠手工小心规避）
//   5. 业务层只依赖 IEventEngine 接口 —— Epoll 版只需新引擎，零改动复用
// ============================================================================
...
//
// 状态与事件关注的关系（不变式，防止 select 空转/漏事件）：
//   * 可读：始终关注（推流/等待期也靠它感知客户端断开）
//   * 可写：当且仅当 out_ 非空 —— 发完立刻撤销，否则 LT 会无限报告可写
//
// 状态：
//   kHead  收请求（含 POST 攒 body）-> 产出响应
//   kWrite 响应没发完，等可写继续冲
//   kLive  直播推流（定时器定帧，永不主动结束）
//   kDelay /slow/ 等待到期（定时器驱动）
// ============================================================================
...
// ----------------------------------------------------------------------------
// 定时驱动（由 HttpServer::Tick 调用；倒序遍历，允许自摘列表）
// ----------------------------------------------------------------------------
