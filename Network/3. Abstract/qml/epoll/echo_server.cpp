...
public:
    EchoSession(EpollEngine& engine, FD fd, long id, const std::string& peer)
    ...
    void OnRead(FD fd, void* data, int32_t mask) override {
        ...
        if (n > 0) {
            ...
            // 有待发数据了：向引擎叠加"可写"关注（引擎内部翻译成 EPOLL_CTL_MOD）
    ...
    void OnWrite(FD fd, void* data, int32_t mask) override {
        ...
        // 发完了：撤销可写关注。LT 模式下若不撤销，内核缓冲一有空位就触发可写，
        // epoll_wait 永远有事件返回，事件循环空转烧 CPU
...
public:
    EchoServer(EpollEngine& engine, FD listenfd)
...
int main() {
    ...
    // ---- 业务方代码：注册监听 fd，跑引擎。与 select 版一字不差 ----
    EpollEngine engine;
    ...
    time_t now = time(NULL);
    strftime(ts, sizeof ts, "%Y-%m-%d %H:%M:%S", localtime(&now));
    printf("[%s] [server] [-] listening on 0.0.0.0:%d (epoll engine)\n", ts, SERV_PORT);
